"""Illustrate measured offline retrieval-preview output, not a fabricated research UI."""
...
def create_gif(transcript_path: Path, output_path: Path) -> None:
    """Render only a validated preview transcript and never overwrite an existing artifact."""
    ...
    if len(panels) != len(PANEL_TITLES) or any(
        ...
        raise ValueError("Expected the four panels produced by the retrieval preview demo.")
    render_panels(
        panels, output_path, banner="SCHOLAR RAG / RETRIEVAL PREVIEW / SYNTHETIC + OFFLINE"
    )
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("docs/assets/retrieval-preview.gif"))
