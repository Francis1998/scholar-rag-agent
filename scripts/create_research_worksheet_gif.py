"""Illustrate measured worksheet output, not a fabricated UI or scientific answer."""

import argparse
from pathlib import Path

from scripts.create_evidence_gif import render_panels
from scripts.demo_research_worksheet import PANEL_TITLES


def create_gif(transcript_path: Path, output_path: Path) -> None:
    """Render only the four worksheet demo panels without replacing an existing artifact."""
    ...
    if len(panels) != len(PANEL_TITLES) or any(
        ...
        raise ValueError("Expected the four panels produced by the worksheet demo.")
    render_panels(
        panels, output_path, banner="SCHOLAR RAG / RESEARCH WORKSHEET / SYNTHETIC + OFFLINE"
    )
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("docs/assets/research-worksheet.gif"))
