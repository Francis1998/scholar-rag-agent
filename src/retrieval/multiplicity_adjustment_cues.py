"""Offline MultiplicityAdjustmentCueExtractor extraction.

Detects advisory cues from title/abstract/methods via deterministic patterns.
Never calls the network. Distinct from PValueHintExtractor and SubgroupAnalysisCueExtractor.
...
_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "bonferroni",
        re.compile(
            r"\bBonferroni\b|\bbonferroni[- ]correct",
            re.IGNORECASE,
        ),
    ),
    (
        "fdr_bh",
        re.compile(
            r"\bfalse discovery rate\b|\bBenjamini[- ]Hochberg\b|\bFDR\b",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"\bmultiplicity (?:adjustment|correction)\b|"
            r"\bmultiple[- ]testing correction\b|"
            r"\bfamily[- ]wise error\b",
            re.IGNORECASE,
        ),
    ),
)
...
class MultiplicityAdjustmentCue:
...
class MultiplicityAdjustmentCueExtractor:
    """Extract offline evidence cues."""

    def extract(self, papers: Sequence[dict[str, object]]) -> tuple[MultiplicityAdjustmentCue, ...]:
        """Return cues for ``papers``."""

        if not papers:
            return ()
        results: list[MultiplicityAdjustmentCue] = []
