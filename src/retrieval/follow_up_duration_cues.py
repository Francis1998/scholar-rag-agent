"""Offline FollowUpDurationCueExtractor extraction.

Detects advisory cues from title/abstract/methods via deterministic patterns.
Never calls the network. Distinct from AttritionRateCueExtractor and SampleSizeHintExtractor.
...
_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "months_follow_up",
        re.compile(
            r"\b(?:\d+\s*)?(?:months?|years?)\s+(?:of\s+)?"
            r"follow[- ]up\b|"
            r"\bfollow[- ]up\s+(?:of\s+)?\d+\s*(?:months?|years?)\b",
            re.IGNORECASE,
        ),
    ),
    (
        "median_follow_up",
        re.compile(
            r"\bmedian follow[- ]up\b|\bmean follow[- ]up\b",
            re.IGNORECASE,
        ),
    ),
    (
        "long_term_follow_up",
        re.compile(
            r"\blong[- ]term follow[- ]up\b|\bextended follow[- ]up\b",
            re.IGNORECASE,
        ),
    ),
)
...
class FollowUpDurationCueExtractor:
    """Extract offline evidence cues."""

    def extract(self, papers: Sequence[dict[str, object]]) -> tuple[FollowUpDurationCue, ...]:
        """Return cues for ``papers``."""

        if not papers:
            return ()
        results: list[FollowUpDurationCue] = []
