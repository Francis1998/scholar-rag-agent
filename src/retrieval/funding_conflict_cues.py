"""Offline FundingConflictCueExtractor extraction.

Detects advisory cues from title/abstract/methods via deterministic patterns.
Never calls the network. Distinct from OpenDataAvailabilityFlagger and RetractionFlagger.
...
_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "industry_funded",
        re.compile(
            r"\bindustry[- ]funded\b|\bsponsored by (?:Pfizer|Novartis|Merck|Roche|AstraZeneca)\b|"
            r"\bpharmaceutical (?:company )?sponsor",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"\bconflicts? of interest\b|\bcompeting interests\b|\bCOI disclosure\b",
            re.IGNORECASE,
        ),
    ),
    (
        "author_employee",
        re.compile(
            r"\bauthors? (?:are|were) employees?\b|\bemployee(?:s)? of the sponsor\b",
            re.IGNORECASE,
        ),
    ),
)
...
class FundingConflictCueExtractor:
    """Extract offline evidence cues."""

    def extract(self, papers: Sequence[dict[str, object]]) -> tuple[FundingConflictCue, ...]:
        """Return cues for ``papers``."""

        if not papers:
            return ()
        results: list[FundingConflictCue] = []
